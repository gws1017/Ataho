...
class Boy:
    ...
    def __init__(self, rand_pos=False):
        ...
        self.targets = []
        self.speed = 0
        if Boy.image == None: #보이의 이미지가 논이면  로드 여러명을 만들어도 한번만 로드할 수 있다.
    ...
    def update(self):
        ...
        if self.target is not None:
            ...
            if self.target == None: # 목표가 있으면 
                print("Removing target: ", self.targets[0], " from %d target(s)." % len(self.targets))
                del self.targets[0] #이동이끝난 웨이포인트 삭제
                if len(self.targets) > 0: #아직남아있으면 갱신
    ...
    def fire(self):
        big = random.choice([False, True]) #큰공 아니면 작은공으로
