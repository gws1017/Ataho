...
class Background:
    def __init__(self, imageName,tp):
        ...
        self.target = None
        if tp == 0:
            self.cw, self.ch = 640, 352
        elif tp == 1:
            self.cw, self.ch = 640, 352
        elif tp == 2:
            self.cw, self.ch = 640, 128
        else:
    ...
    def draw(self):
        ...
        if self.tp == 0 or self.tp == 1:
            self.image.clip_draw_to_origin(*self.win_rect, 0, 128,640,352)
        elif self.tp == 2:
...
class FixedBackground(Background):
    MARGIN_L, MARGIN_B, MARGIN_R, MARGIN_T = 20, 128, 20, 30
    def __init__(self, imageName,tp):
        super().__init__(imageName,tp)
        self.boundary = (
            FixedBackground.MARGIN_L, 
            FixedBackground.MARGIN_B,
            self.image.w - FixedBackground.MARGIN_R, 
            self.image.h - FixedBackground.MARGIN_T + FixedBackground.MARGIN_B
        )
