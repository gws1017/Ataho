...
6. 개발 범위
   ...
   | 오브젝트 |              석상(세이브 포인트), ~~상점 NPC~~               | <span style="color:red">상점NPC</span>,숨겨진 오브젝트(아이템) |
   |  아이템  |    ~~회복 아이템~~, <span style="color:red">**X**</span>     | ~~장비 아이템, 상태회복~~,<span style="color:red">**회복 아이템**</span> |
...
8. **개발 일정**
   ...
   | 주차  | 구현 항목                                              | 세부 내용                                                  |
   | ----- | ------------------------------------------------------ | ---------------------------------------------------------- |
   | 1주차 | <span style="color:red">(90%)</span>리소스             | 리소스 배경색 투명으로 바꾸기, 맵그리기(마을,필드)         |
   | 2주차 | <span style="color:red">(100%)</span>오브젝트          | 캐릭터 이동 ,스텟(HP,MP,EXP,공격력,레벨 등), ~~아이템~~    |
   | 3주차 | <span style="color:red">(100%)</span>점검 및 기본 토대 | 게임 프레임워크 마련,  부족한점 보완                       |
   | 4주차 | <span style="color:red">(100%)</span>전투              | 전투 체계 수립(전투 시작 조건, 턴 돌아가는 방식,몬스터 AI) |
   | 5주차 | <span style="color:red">(100%)</span>애니메이션        | 기술 애니메이션 구현 , 일반 몬스터 애니메이션 구현         |
   | 6주차 | <span style="color:red">(0%)</span>보스                | 보스 애니메이션, 상호작용, 대사, 보스 AI                   |
   | 7주차 | <span style="color:red">(10%)</span>기타기능           | ~~상점~~, 저장, 설정화면, 사운드 구현, 엔딩 화면           |
...
9. **주차별 커밋 통계**
   ...
   <img src="https://user-images.githubusercontent.com/65538479/99866981-1e4ac600-2bf9-11eb-9a9f-180fdf25186d.JPG" alt="캡처3" style="zoom: 80%;" />
...
10.  **메인스테이트의 게임 오브젝트 설명**
    ...
    **player.wmax = randint(40,80)**
    ...
    걸음수/ 최대 걸음수 를 지정해주고(최대 걸음수는 항상 다름)

    걸음수가 최대걸음수만큼 차면 전투에 진입하게됩니다.

    전투에서는 **from battle_manage import BattleManager**

    모듈을 임포트하여 bm = BattleManager() 이라는 객체가 플레이어와 적의 전투를 관리합니다.

    **class BattleManager** 에는 

    플레이어 정보, 적 정보(self.player = Player() ,self.monster =  Monster(0)) 를 갖고있고

    **def update(self)** 함수와 **def handle_event(self, e)** 함수에서 정보,키 입력에 따라서 처리를
    해줍니다.

11.  **구현하면서 특히 어려웠던/어려운 부분, 수업에서 추가로 다루었으면 하는 것**

    전투의 턴제방식을 구현할때 제가 할때는 거의 강제적으로 플레이어가 공격이끝났으면

    무조건 적이 공격한다. 그 다음턴 시작이런 식이라 

    적의 수 증가 또는 플레이어가 관리하는 캐릭터수 증가(동료) 이러한 경우가 되면 

    제대로 돌아가지 않게되는데 정확한 턴제 전투의 방식을 배우고 싶습니다.
